...
class Knn:
	...
	def distance(self, point):
		...
		for i, series in labeled.iterrows():
			...
			newSeries = newSeries.map(lambda x: x**100)
			toRoot = newSeries.sum() #the number to root
			final = toRoot**(.01) #the final distance from the labeled point to the unlabeled
	...
	def classifyOne(self, point):
		...
		for index, row in self.classFrame.head(n=10).iterrows():
	...
	def classifyAll(self):
		...
		for index, point in self.unlabeled.iterrows():
			...
			answers.append(self.classifyOne(point))
